...
export class PharmaNewsService {
  ...
  private extractGeopoliticalAnalysis(content: string): string {
    const geoKeywords = ['global', 'international', 'regulatory', 'policy', 'trade', 'access'];
    ...
    if (!geoAnalysis || geoAnalysis.length < 200) {
      geoAnalysis = `Current global pharmaceutical landscape shows continued emphasis on regulatory harmonization and international cooperation in drug development. Today's developments across major regulatory authorities demonstrate ongoing commitment to patient access and safety standards. International pharmaceutical trade dynamics support continued innovation and market access across multiple jurisdictions.`;
    }
